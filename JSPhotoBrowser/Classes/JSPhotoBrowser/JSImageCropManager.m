...
@implementation JSImageCropManager
...
//        [path appendPath:[UIBezierPath bezierPathWithArcCenter:containerView.center radius:cropRect.size.width / 2 startAngle:0 endAngle: 2 * M_PI clockwise:NO]];
        [path appendPath:[UIBezierPath bezierPathWithArcCenter:CGPointMake(containerView.center.x, containerView.center.y) radius:cropRect.size.width / 2 startAngle:0 endAngle: 2 * M_PI clockwise:NO]];
    } else { // 矩形裁剪框
...
+(UIImage *)cropImageView:(UIImageView *)imageView toRect:(CGRect)rect zoomScale:(double)zoomScale containerView:(UIView *)containerView
{
    ...
    CGFloat xMargin = containerView.js_width - CGRectGetMaxX(rect) - rect.origin.x;
